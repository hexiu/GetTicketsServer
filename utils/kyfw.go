...
func (kyfw *Kyfw) AddQueryScheduleLog(startStation, endStation, startCode, endCode, date string) error {
	//设置Cookie
	beego.Debug("insert data: ", startStation, startCode, endStation, endCode, date)
	cookies := []*http.Cookie{
		...
		&http.Cookie{Name: "RAIL_DEVICEID", Value: "ZmJ6xAxFmNanf1k9oIvCakvWMJMhdlFw0DDjgLeYtKptQtEQT_6j4TL7MKy6l_BG1z4WHIZ2g7fzqJwztQd6nB8xsCvpjWUa9iXPWZgWxiAL-iBzE1OOl5L-l3oe7M0Yo8qvOz_OXuQkUQ9KBOYunKh6RXIXNZBw"},
		&http.Cookie{Name: "RAIL_EXPIRATION", Value: "1545853272102"},
	}
	kyfw.Request.CreateHttpRequest(fmt.Sprintf(QueryScheduleLog, date, startCode, endCode), "GET", nil)
	errCookies := kyfw.Request.SetCookie(cookies)
	loger.Println("set cookies err: ", errCookies)
	kyfw.Request.SetHeader("X-Requested-With", "XMLHttpRequest")
	loger.Println(kyfw.Request.HttpRequest.Cookies(), cookies)
	data, err := kyfw.Request.Send()
	if err != nil {
		return errors.New("车次查询日志添加失败")
	}
	beego.Debug("add Query: ", string(data))
	return nil
}
...
func (kyfw *Kyfw) QueryScheduleInfo(date, startCode, endCode string) (string, error) {
	...
	//没有取到数据时递归调用直到取到数据为止
	if len(data) == 0 {
		return "", errors.New("获取车次信息失败" + strconv.Itoa(kyfw.MaxLoopNumber))
	}
	beego.Info("QuerySchedule:", string(data))
...
func (kyfw *Kyfw) GetSchedule(startStation, endStation, startCode, endCode, date string) (string, error) {
	...
	for kyfw.QueryNumber <= kyfw.MaxLoopNumber {
		kyfw.QueryNumber++
		//添加查询日志
		...
		//查询车次信息
		data, errQuery := kyfw.QueryScheduleInfo(date, startCode, endCode)
		if errQuery != nil {
			continue
		}
		if errQuery == nil {
	...
	kyfw.QueryNumber = 0
	return "", errors.New("GetSchedule Error! ")
...
func (kyfw *Kyfw) PlaceAnOrder(secret, trainNo, trainCode, start, startCode, end, endCode, date, formatDate, ticketStr, passengerStr string) error {
	//提交订单
	cookies := kyfw.Request.HttpRequest.Cookies()
	loger.Println("down:", cookies)
...
func (kyfw *Kyfw) InitConfirmOrder() error {
	...
	beego.Debug("InitConfirmOrder: ", string(data))
	//获取KeyCheck
...
func (kyfw *Kyfw) GetTicketCode() error {
	...
	if tickData["waitTime"].(float64) < 0 {
		switch tickData["requestId"].(type) {
		case nil:
			return errors.New(fmt.Sprintln(tickData))
		case string:
			kyfw.OrderTicketCode = tickData["requestId"].(string)
			return nil
		}
	} else {
		return kyfw.GetTicketCode()
