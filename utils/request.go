...
func ReadHttpResponseData(request *Request) ([]byte, error) {
	...
	//返回读取数据
	body, err := ioutil.ReadAll(request.HttpResponse.Body)
	loger.Println("request:", request.HttpRequest)
	loger.Println("resp:", request.HttpResponse)
	loger.Println(string(body), err)
	return body, err
}
...
func (request *Request) Send() ([]byte, error) {
	if request.HttpRequest != nil {
		loger.Println("URL: ", request.HttpRequest.URL.String())
		loger.Println("Cookies: ", request.HttpRequest.Cookies())
		//设置header
